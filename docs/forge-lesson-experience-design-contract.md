...
Is the overall lesson still technically serious? 37. Visual Authoring Checklist
...
Is the visual still understandable without motion? 38. Quality Checklist
Pedagogy
...
Animations do not cause interaction lag. 39. Testing Contract
...
Integration
The demo must prove:
definition → controller → renderer → interaction → validation/runtime → controller → progression → completion 40. Demo Requirements
...
It must not become the hidden production curriculum. 41. Demo Independence Requirements
The demo must prove:
...
The controller does not know the concept being taught. 42. Production Boundary
...
The new experience engine must not silently replace them. 43. Non-Goals
...
It does not authorize curriculum production. 44. Explicit Architectural Invariants
...
The new experience architecture must not be used merely to recreate the old document-style lesson UI. 45. Future Authoring Pipeline
...
rather than:
how the React component should render it. 46. Future Content Generation
...
content creation
separate from:
application architecture. 47. What Makes a Forge Lesson Different
...
The underlying learning philosophy should not. 48. Decision Gate Before Real Curriculum Authoring
...
Do not compensate for architectural uncertainty by writing more curriculum. 49. Current Proof Surface
...
The answer must be demonstrated through actual interaction, not screenshots or static mockups. 50. Final Design Principle
...
That is the standard every future Forge lesson should be judged against. 51. Implementation Boundary
...
The next phase after architectural acceptance is curriculum authoring against this contract, not further modification of the legacy lesson model. 52. Acceptance Statement
