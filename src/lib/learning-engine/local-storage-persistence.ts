...
function isValidPersistedSession(value: unknown): value is LessonSessionState {
  ...
  if (new Set(order).size !== order.length || !order.includes(session.currentActivityId))
    return false;
  ...
  const activityMap = session.activities as Record<
    string,
    Partial<LessonSessionState["activities"][string]>
  >;
