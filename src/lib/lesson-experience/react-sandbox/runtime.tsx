...
import {
  ...
  RUNTIME_LIMITS,
  type RuntimeExecuteMessage,
  type RuntimeDisposeMessage,
} from "./protocol";
...
export function ReactSandboxRuntime() {
  useEffect(() => {
    ...
    const blocked = () =>
      Promise.reject(new Error("Network access is disabled in the React sandbox."));
    for (const [key, value] of [
      ...
      try {
        Object.defineProperty(window, key, { configurable: false, value, writable: false });
      } catch {
        /* browser-owned globals may already be locked */
      }
    }
    try {
      Object.defineProperty(navigator, "sendBeacon", { configurable: false, value: () => false });
    } catch {
      /* browser-owned APIs may already be locked */
    }
    const execute = (message: RuntimeExecuteMessage) => {
      if (
        runtimeWindow.__forgeDisposed ||
        activeRequestId ||
        message.nonce !== nonce ||
        message.runId !== runId ||
        ...
        return;
      if (
        message.source.length > RUNTIME_LIMITS.sourceBytes ||
        JSON.stringify(message.props).length > RUNTIME_LIMITS.propsBytes
      )
        return;
      ...
      try {
        console.log = (...args) => {
          if (logs.length < RUNTIME_LIMITS.consoleEntries)
        ...
        runtimeWindow.__forgeRoot.render(
        ...
        window.setTimeout(() => {
          post({
            ...
            dom: rootHost.innerHTML.slice(0, RUNTIME_LIMITS.domBytes),
            runtimeError: null,
            console: logs,
            renderCount,
          });
    ...
    const onMessage = (event: MessageEvent<RuntimeExecuteMessage | RuntimeDisposeMessage>) => {
      ...
      if (
        !message ||
        message.protocolVersion !== REACT_RUNTIME_PROTOCOL_VERSION ||
        message.nonce !== nonce
      )
        return;
      if (message.type === "runtime:dispose") {
        runtimeWindow.__forgeDisposed = true;
        runtimeWindow.__forgeRoot?.unmount();
