...
import {
  createCanonicalRuntimeManifest,
  type CanonicalRuntimeActivity,
} from "./canonical-code-runtime";
import {
  validationReportToActivityResult,
  createRuntimeErrorReport,
} from "./canonical-validation-adapter";
...
function toAssertion(
  activity: RuntimeSourceActivity,
  test: { id?: string; description: string; assertion?: string },
  index: number,
) {
  const base = {
  ...
  if (activity.content.language === "css") {
    ...
    return {
      ...base,
      strategy: "computed_style" as const,
      target: match?.[1] ?? ".navbar",
      expected: { property: match?.[2] ?? "display", value: match?.[3] ?? "flex" },
    };
  }
  ...
  if (jsMatch) {
    ...
    try {
      ...
      expectedValue = raw.replace(/^['"]|['"]$/g, "");
    }
  }
  return {
    ...base,
    strategy: "js_evaluation" as const,
    expected: { expression: jsMatch?.[1]?.trim() ?? test.assertion ?? "false", expectedValue },
  };
}
...
export function createCanonicalValidationSpec(
  ...
  const assertions = (activity.content.testCases ?? []).map((test, index) =>
    toAssertion(activity, test, index),
  );
  const hasSelectorChallenge =
    activity.content.language === "html" &&
    /getElementById\(['"](?:wrong-button-id|submit-btn)['"]\)/.test(source) &&
    source.includes('id="submit-btn"');
  if (hasSelectorChallenge) {
    assertions.push({
      id: "runtime-status",
      description: "Clicking Launch App updates the status",
      strategy: "dom_query",
      target: "#status",
      expected: { exists: true },
      failureMessage: "The button should update the status after it is clicked.",
    });
  }
  return {
    ...
    runtime,
    assertions,
    ...(hasSelectorChallenge
      ? { actions: [{ type: "click" as const, selector: "#submit-btn" }] }
      : {}),
  };
}
...
export function createCanonicalValidationRequest(
  ...
  return {
    ...
    requestId: `canonical-${activity.id}-${revision}-${Date.now()}`,
    exerciseId: activity.id,
    validationSpec: createCanonicalValidationSpec(activity, source),
