...
describe("canonical runtime adapter", () => {
  it("creates deterministic HTML manifests", () => {
    ...
    expect(createCanonicalRuntimeManifest(input)).toMatchObject({
      ...
      manifest: {
        runtime: "vanilla-dom",
        entryFile: "index.html",
        files: [{ name: "index.html", code: input.source }],
      },
    });
  });

  it("represents an explicit HTML/CSS fixture", () => {
    const result = createCanonicalRuntimeManifest({
      id: "css-1",
      runtime: "html-css",
      fixture: "<nav />",
      source: "nav { display: flex; }",
    });
    expect(result).toMatchObject({
      kind: "html-css",
      cssFile: "styles.css",
      manifest: { runtime: "html-css" },
    });
  ...
  it("represents HTML plus JavaScript explicitly", () => {
    const result = createCanonicalRuntimeManifest({
      ...
      fixture: "<button id='go'>Go</button>",
      source: "document.querySelector('#go');",
    });
    expect(result).toMatchObject({
  ...
  it("rejects runtime descriptions missing required explicit fixtures", () => {
    expect(() =>
      createCanonicalRuntimeManifest({ id: "missing", runtime: "html-css", source: "body {}" }),
    ).toThrow(CanonicalRuntimeAdapterError);
    expect(() =>
      createCanonicalRuntimeManifest({
        id: "missing",
        runtime: "html-javascript",
