...
describe("canonical validation adapter", () => {
  it("maps a passing report to valid canonical evidence", () => {
    const result = validationReportToActivityResult({
      ...
      status: "passed",
      results: [
        { assertionId: "required", description: "required", status: "passed", durationMs: 1 },
      ],
      passedCount: 1,
      totalRequired: 1,
      timestamp: 1,
  ...
  it("maps failed and runtime reports to invalid results", () => {
    ...
    expect(validationReportToActivityResult(createRuntimeErrorReport("x", "boom")).isValid).toBe(
      false,
    );
  });
});
