  ...
  describe("5. End-to-End Learning Engine Progression & Completion Lifecycle", () => {
      ...
      for (const activity of lesson.activities) {
        if (activity.id === judgmentActivity.id && score !== undefined) {
          session = engageSessionActivity(
            session,
            activity.id,
            "A sufficiently detailed judgment response that explains the evidence and next investigation.",
          );
    ...
    it("requires a scored evaluation for a 100% minimum-score lesson", () => {
      ...
      expect(check.reasons).toContain(
        "Minimum score of 100% cannot be verified without scored activities",
      );
    });
    ...
    it("rejects a judgment score below the 100% minimum", () => {
      ...
      expect(check.canComplete).toBe(false);
      expect(
        check.reasons?.some((reason) => reason.includes("Minimum score of 100% not met")),
      ).toBe(true);
    });
    ...
    it("progresses through all 5 activities including judgment, records evidence, and achieves lesson completion", () => {
      ...
      session = resolveActivityEvaluation(session, "act-144-judgment-diagnose", {
        ...judgmentResult,
        score: 100,
      });
